"use client";
/**
 * components/ManageCollaborators.jsx
 * ─────────────────────────────────────────────────────────────────────────────
 * Owner-only panel for authorizing collaborator wallet addresses.
 * Automatically hides itself if the connected wallet is not the contract owner.
 *
 * Props
 * ─────
...
// Minimal validation: Ethereum address must be 42 chars starting with 0x
...
export default function ManageCollaborators({ contractAddress, contractABI, projectId }) {
  ...
  // Recently authorized addresses in this session (for the success list)
  ...
  useEffect(() => {
    ...
    // Re-check if user switches MetaMask account
    if (window.ethereum) {
  ...
  const handleAddressChange = (e) => {
    ...
    if (val && !isValidAddress(val)) {
      setAddrError("Must be a valid Ethereum address (0x + 40 hex chars)");
    } else {
      setAddrError("");
    }
  };

  // ── Submit: authorizeCollaborator ────────────────────────────────────────
  const handleAuthorize = async (e) => {
    ...
    try {
      ...
      toast.success(`Collaborator authorized!`, {
        id: toastId,
        icon: "✓",
        style: {
          background: "#0d0d0d",
      ...
      // Add to session list and clear input
  ...
  // ── Not owner: show a subtle locked state ────────────────────────────────
  if (!isOwner) {
    return (
      <div className="font-mono bg-[#0d0d0d] border border-[#1a1a1a] rounded-2xl px-6 py-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[#2a2a2a] text-[10px] mt-0.5">
              Connected: <span className="text-[#333] font-mono">{connectedWallet ? `${connectedWallet.slice(0,6)}…${connectedWallet.slice(-4)}` : "—"}</span>
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="font-mono">
      <div className="relative bg-[#0d0d0d] border border-[#2a2a2a] rounded-2xl overflow-hidden shadow-2xl shadow-black/60">
        {/* Accent bar — amber for admin/owner panel */}
        ...
        <div className="px-6 py-5">
          ...
          <div className="flex items-start justify-between mb-5">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <p className="text-[9px] tracking-[0.3em] uppercase text-[#f59e0b]">
                  Owner Panel
                </p>
                <span className="text-[9px] font-bold tracking-widest uppercase text-[#f59e0b] bg-[#f59e0b]/10 border border-[#f59e0b]/20 px-2 py-0.5 rounded-full">
                  Admin
                </span>
              </div>
          ...
          <form onSubmit={handleAuthorize} className="space-y-4">
            <div>
              ...
              <input
                ...
                spellCheck={false}
                className={`
                  w-full bg-[#0a0a0a] border rounded-xl px-4 py-3
                  font-mono text-sm text-white placeholder-[#2a2a2a]
                  focus:outline-none transition-colors duration-150
                  disabled:opacity-50 disabled:cursor-not-allowed
                  ${addrError
                    ? "border-red-900/50 focus:border-red-700/60"
                    : address && isValidAddress(address)
                      ? "border-[#f59e0b]/30 focus:border-[#f59e0b]/60"
              ...
              {address && isValidAddress(address) && !addrError && (
                <p className="text-[#f59e0b]/60 text-[10px] mt-1.5 flex items-center gap-1">
                  <span>✓</span> Valid Ethereum address
                </p>
              )}
            </div>
            ...
            <button
              type="submit"
              disabled={isSubmitting || !!addrError || !address}
              className={`
                w-full py-3.5 rounded-xl text-[11px] font-bold tracking-[0.2em] uppercase
                transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed
                ${isSubmitting
                  ? "bg-[#111] border border-[#222] text-[#555]"
                  : "bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-[#050505] hover:brightness-110 active:scale-[0.99] shadow-lg shadow-[#f59e0b]/10"
                }
              `}
            >
              {isSubmitting ? (
          ...
          {/* ── This session's authorized addresses ─────────────────────── */}
          {authorized.length > 0 && (
            <div className="mt-5 pt-4 border-t border-[#111]">
              <p className="text-[9px] tracking-[0.2em] uppercase text-[#444] mb-3">
          ...
          {/* Info note */}
          <p className="text-center text-[10px] text-[#222] mt-4 leading-relaxed">
            Authorized wallets can log contributions to this project.
            <br />Revocation must be done via the smart contract directly.
          </p>
        </div>
      </div>
    </div>
